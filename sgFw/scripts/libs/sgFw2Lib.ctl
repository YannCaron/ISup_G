...
//			sgFwSubDisabledParent(parents[cpt], systemName, disabledValue, subDisabledValue);
			...
			for(int cptInputs = 1; cptInputs <= dynlen(inputs); cptInputs++)
			{
				...
				if(disabled || subDisabled)
				{
					disabledInputFound = true;
					if(!parentsSubDisabled[cpt])
					{
						sgHistoryAddEvent("FwUtils.SystemHistory", SEVERITY_SYSTEM, "System " + parents[cpt] + " sub-disabled because input " + inputs[cptInputs] + " is disabled or sub-disabled");
